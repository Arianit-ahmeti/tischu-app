  ...
  const names = () => {
    if (dataLoading) {
      ...
      return (
        <ColumnView>
          <RowView>
            <ThemedText style={styles.marginVert} variant='h4'>Von</ThemedText>
            <ThemedText  variant='h4'>{userName || "Unbekannt"}</ThemedText>
          </RowView>
          <RowView>
            <ThemedText style={styles.marginVert} variant='h4'>Tier</ThemedText>
  ...
  return (
    <Pressable style={styles.card}
      ...
      <RowView style={styles.element}>
        <ColumnView style={styles.text}>
          {names()}
          </ColumnView>
        <ColumnView>{(props.message.read_at?.valueOf() == null) && (<Chip text="Neu"/>)}</ColumnView>
        <ColumnView><IconButton onPress={() => {
          Alert.alert("Tier löschen", `Möchten Sie die Kontaktanfrage von ${userName} für ${animalName} wirklich löschen?`, [
            {
              text: "Abbrechen",
              style: "cancel",
            },
            ...
            }])}} iconSet='Feather' iconName='trash' /></ColumnView>
      </RowView>
    </Pressable>)
}
...
const styles = StyleSheet.create({
  card: {backgroundColor: theme.colors.background.warm, borderColor: theme.colors.border.light, borderWidth:1, flexWrap: "nowrap", height: 75, margin:5, borderRadius:10, paddingLeft:10},

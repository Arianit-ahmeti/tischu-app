import { AlertDialog, ColumnView, RowView, SelectableButton, ThemedArrayInput, ThemedButton, ThemedText, ThemedTextInput } from '@components';
...
export default function AdoptionFormUserSituation() {
  ...
  async function getData() {
    ...
    if (!animalType) {
      ...
      switch (type) {
        case "dog": setAnimalType("en Hund");
        case "cat": setAnimalType("e Katze");
        case "turtle": setAnimalType("e Schildkröte");
        case "horse": setAnimalType(" Pferd");
      }
    ...
    <AlertDialog
      ...
      <ScrollView>
        ...
        <View style={styles.selectionContainer}>
                    ...
                    {situation.experience && situation.experience != "none" &&(<View>
            <ThemedText variant='h3'>Bitte beschreiben Sie ihre vergangenen Erfahrungen genauer</ThemedText>
            <ThemedText variant='bodyLarge'>Geben Sie bitte pro Tier folgende Daten an: Rasse? Alter? Wie lange lebte das Tier bei Ihnen? Weitere Informationen?</ThemedText>

            {situation.experience == "one"&& (<View><ThemedTextInput value={situation.past_animals?.at(0) || ""}
              onChangeText={(text) => setSituation({ ...situation, past_animals: [text] })}
              placeholder="Tier"/>
            </View>)}
            {situation.experience != "one" && (<ThemedArrayInput data={situation.past_animals} labels='Tier' onChange={(text) => setSituation({ ...situation, past_animals: text })}/>)}
          </View>)
          }
          ...
          <ThemedTextInput value={situation.household_size?.toString() || ""}
            keyboardType='numeric'
            onChangeText={(text) => setSituation({ ...situation, household_size: parseInt(text) || null })}
            placeholder="Anzahl Personen" />

          {(situation.household_size && situation.household_size>1) && (<View><ThemedText variant='h3'>Leben Kinder im Haushalt?</ThemedText><View style={styles.selectionContainer}><SelectableButton key="ChildrenTrue"
                        isSelected={situation.children?.valueOf()==true}
                        onPress={() => setSituation({ ...situation, children: true })}
                        style={styles.selectableButton}
                      >Ja</SelectableButton><SelectableButton key="ChildrenFalse"
                        isSelected={situation.children?.valueOf()==false}
                        onPress={() => setSituation({ ...situation, children: false })}
                        style={styles.selectableButton}
            >Nein</SelectableButton></View>
            {(situation.children?.valueOf() == true && (<View><ThemedText variant='h3'>Wie alt sind die Kinder?</ThemedText><ThemedArrayInput data={situation.children_ages} labels='Kind' onChange={(text) => setSituation({ ...situation, children_ages: text })} singleRow={true} /></View>))}
          </View>)}

          <ThemedText variant='h3'>Leben weitere Tiere im Haushalt?</ThemedText>
          <View style={styles.selectionContainer}>
          <SelectableButton key="AnimalsTrue"
                        isSelected={moreAnimals.valueOf()==true}
                        onPress={() => setMoreAnimals(true)}
                        style={styles.selectableButton}
                      >Ja</SelectableButton><SelectableButton key="AnimalsFalse"
                        isSelected={moreAnimals.valueOf()==false}
                        onPress={() => setMoreAnimals(false)}
                        style={styles.selectableButton}
          >Nein</SelectableButton></View>

          {moreAnimals.valueOf() == true && (<View>
            <ThemedText variant='h3'>Bitte beschreiben Sie die Tiere, die im Haushalt leben, genauer</ThemedText>
            <ThemedText variant='bodyLarge'>Geben Sie bitte pro Tier folgende Daten an: Tierart? Rasse? Alter? Wie lange lebt das Tier schon bei Ihnen? Weitere Informationen?</ThemedText>
            <ThemedArrayInput data={situation.current_animals} labels='Tier' onChange={(text) => setSituation({ ...situation, current_animals: text })}/>
          </View>)}

          <ThemedText variant='h3'>Leben Sie in einem Haus oder einer Wohnung?</ThemedText>
          <View style={styles.selectionContainer}><SelectableButton key="HouseTrue"
                        isSelected={situation.living_house?.valueOf()==true}
                        onPress={() => setSituation({ ...situation, living_house: true })}
                        style={styles.selectableButton}
                      >Haus</SelectableButton><SelectableButton key="HouseFalse"
                        isSelected={situation.living_house?.valueOf()==false}
                        onPress={() => setSituation({ ...situation, living_house: false })}
                        style={styles.selectableButton}
            >Wohnung</SelectableButton></View>

          {situation.living_house?.valueOf() == false && (<View>
          ...
          <View style={styles.selectionContainer}><SelectableButton key="MovingTrue"
                        isSelected={situation.moving_plans?.valueOf()==true}
                        onPress={() => setSituation({ ...situation, moving_plans: true })}
                        style={styles.selectableButton}
                      >Ja</SelectableButton><SelectableButton key="MovingFalse"
                        isSelected={situation.moving_plans?.valueOf()==false}
                        onPress={() => setSituation({ ...situation, moving_plans: false })}
                        style={styles.selectableButton}
            >Nein</SelectableButton></View>

          <ThemedText variant='h3'>Wie viel Zeit müsste das Tier an einem durchschnittlichen Tag alleine verbringen?</ThemedText>
          ...
          <ThemedTextInput
            ...
            <ThemedText style={styles.hours}>Stunden pro Tag</ThemedText>
                    ...
                    <ThemedText variant='h3'>Haben Sie sichergestellt, dass Sie die finanziellen Mittel für ein Tier haben?</ThemedText>
          <ThemedText variant='bodyLarge'>Bedenken Sie nicht nur regelmäßige Kosten, wie Futter, sondern auch unvorhergesehene, hohe Kosten, wie Tierarztbesuche und Medikamente</ThemedText>
          <View style={styles.selectionContainer}><SelectableButton key="FinancesTrue"
                        isSelected={situation.financial_situation?.valueOf()==true}
                        onPress={() => setSituation({ ...situation, financial_situation: true })}
                        style={styles.selectableButton}
                      >Ja</SelectableButton><SelectableButton key="FinancesFalse"
      ...
      <View style={styles.buttonContainer}>
        <ThemedButton
          ...
          disabled={saving}>Senden</ThemedButton>
      </View>
    </SafeAreaView>
  </>);
